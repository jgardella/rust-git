...
fn collect_items_to_hash(cmd: &HashObjectCommand) -> Result<Vec<Box<dyn BufRead>>, RustGitError> {
    ...
    if cmd.args.stdin_paths {
        let stdin_files: Vec<File> = io::stdin()
            .lock()
            .lines()
        ...
        stdin_files
            .into_iter()
            .for_each(|file| to_hash.push(Box::new(BufReader::new(file))));
...
impl GitCommand for HashObjectCommand {
    fn execute(&self, repo_state: RepoState) -> Result<(), RustGitError> {
        ...
        if self.args.no_filters {
            return Err(RustGitError::new(String::from(
                "--no-filters not supported",
            )));
        }
        ...
        to_hash
            ...
            .map(|br| {
                ...
                repo.hash_obj(self.args.object_type, s, self.args.write)
